...
1. 프리미엄 결과 하단: 명리 입문서, 사주 관련 도서, 노트, 다이어리
2. 블로그 연결 구간: 이현 블로그의 상세 글과 연계되는 상품
3. 명식표 화면 하단: 공부용 도서와 필기 도구

결과 상단과 핵심 지표 사이에는 광고를 넣지 않습니다.

팝업을 쓸 경우에는 결과 열람을 방해하는 전면 차단형이 아니라, 결과를 본 뒤 선택적으로 쿠팡으로 이동하는 전환 팝업으로 설계합니다.
